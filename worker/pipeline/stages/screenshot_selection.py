...
from worker.media.video_frame_extractor import ExtractedFrameCandidate


def apply_selected_limit(roles: list[str], *, selected_count: int) -> list[str]:
    if not roles:
        return []
    max_selected = max(1, selected_count)
...
def select_step_screenshot_slots(*, step: StepRecord, candidate_screenshots: list[ScreenshotCandidateRecord], selected_count: int) -> list[DerivedScreenshotRecord]:
    if not candidate_screenshots:
        return []
    roles = apply_selected_limit(select_screenshot_roles(step), selected_count=selected_count)
    ...
    for sequence_number, role in enumerate(roles, start=1):
        ...
        scoped_candidates = [
            candidate for candidate in candidate_screenshots
