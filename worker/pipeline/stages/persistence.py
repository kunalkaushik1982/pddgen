...
class PersistenceStage:
    ...
    def run(self, db, context: DraftGenerationContext) -> dict[str, int | str]:  # type: ignore[no-untyped-def]
        with bind_log_context(stage="persistence"):
            ...
            context.inputs.session.overview_diagram_json = context.overview_diagram_json
            context.inputs.session.detailed_diagram_json = context.detailed_diagram_json
            if not context.inputs.include_diagram:
                process_groups = (
                    db.execute(
                        select(ProcessGroupModel).where(
                            ProcessGroupModel.session_id == context.inputs.session_id,
                        )
                    )
                    .scalars()
                    .all()
                ...
                for process_group in process_groups:
                    process_group.overview_diagram_json = context.overview_diagram_json
                    process_group.detailed_diagram_json = context.detailed_diagram_json

            step_models = [ProcessStepModel(session_id=context.inputs.session_id, **to_step_record(step)) for step in context.all_steps]
            db.add_all(step_models)
            db.flush()
            persist_step_screenshots(db, step_models=step_models, step_candidates=context.all_steps)
            db.add_all(ProcessNoteModel(session_id=context.inputs.session_id, **to_note_record(note)) for note in context.all_notes)
            pending_bundles = (
                db.execute(
                    select(MeetingEvidenceBundleModel).where(
                        MeetingEvidenceBundleModel.session_id == context.inputs.session_id,
