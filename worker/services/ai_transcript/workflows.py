...
def infer_process_group(
    *,
    ...
    if not process_title:
        return None
    return ProcessGroupInterpretation(process_title=process_title, canonical_slug=canonical_slug, matched_existing_title=matched_existing_title)
...
def resolve_ambiguous_process_group(
    *,
    ...
    parsed = parse_json_object(extract_content(_post_json(settings=settings, payload=payload, context="ambiguous process-group resolution")))
...
def classify_workflow_boundary(*, settings: Any, left_segment: dict[str, Any], right_segment: dict[str, Any]) -> WorkflowBoundaryInterpretation | None:
    ...
    parsed = parse_json_object(extract_content(_post_json(settings=settings, payload=payload, context="workflow boundary classification")))
...
def match_existing_workflow_group(
    *,
    ...
    return WorkflowGroupMatchInterpretation(
        matched_existing_title=normalize_existing_title(str(parsed.get("matched_existing_title", "") or ""), existing_titles=[str(group.get("title", "") or "") for group in existing_groups]),
        recommended_title=recommended_title,
    ...
    *,
    ...
    payload = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": json.dumps({"transcript_name": transcript_name or "", "segment_context": segment_context or {}, "segment_text": segment_text}, ensure_ascii=False),
            },
        ],
    }
    parsed = parse_json_object(extract_content(_post_json(settings=settings, payload=payload, context="workflow semantic enrichment")))
    ...
    filled_fields = sum(
        1
        for value in (
            parsed.get("actor"), parsed.get("actor_role"), parsed.get("system_name"), parsed.get("action_verb"),
            parsed.get("action_type"), parsed.get("business_object"), parsed.get("workflow_goal"),
        )
        if normalize_optional_text(value)
    )
    confidence = calibrate_confidence(normalize_confidence(parsed.get("confidence")), evidence_points=filled_fields, quality_points=len(domain_terms) + len(rule_hints))
...
def summarize_process_group(
    *,
                ...
                "content": json.dumps(
                    {
                        ...
                        "workflow_summary": workflow_summary,
                        "steps": [{"application_name": step.get("application_name", ""), "action_text": step.get("action_text", ""), "supporting_transcript_text": step.get("supporting_transcript_text", "")} for step in steps[:12]],
                        "notes": [{"text": note.get("text", ""), "inference_type": note.get("inference_type", "")} for note in notes[:6]],
                    },
                    ensure_ascii=False,
                ),
            },
    ...
    if not summary_text:
        return None
    confidence = calibrate_confidence(normalize_confidence(parsed.get("confidence")), evidence_points=workflow_evidence_points(workflow_summary), quality_points=summary_quality_points(summary_text))
    return ProcessSummaryInterpretation(summary_text=summary_text, confidence=confidence, rationale=str(parsed.get("rationale", "") or "").strip())
    ...
    *,
    ...
    payload = {
        ...
        "messages": [
            ...
            {
                "role": "user",
                "content": json.dumps({"document_type": document_type, "process_title": process_title, "workflow_summary": workflow_summary}, ensure_ascii=False),
            },
        ],
    }
    parsed = parse_json_object(extract_content(_post_json(settings=settings, payload=payload, context="workflow capability classification")))
    capability_tags = normalize_label_list(parsed.get("capability_tags", []), max_items=3, exclude={process_title})
    confidence = calibrate_confidence(normalize_confidence(parsed.get("confidence")), evidence_points=workflow_evidence_points(workflow_summary), quality_points=len(capability_tags))
    return WorkflowCapabilityInterpretation(capability_tags=capability_tags, confidence=confidence, rationale=str(parsed.get("rationale", "") or "").strip())
