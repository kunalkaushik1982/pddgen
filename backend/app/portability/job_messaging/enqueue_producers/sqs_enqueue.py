...
class SqsJobEnqueueAdapter(JobEnqueuePort):
    """`SendMessage` with envelope JSON and logical queue -> URL mapping support."""

    __slots__ = ("_client", "_settings", "_queue_url", "_queue_urls")
    ...
    def __init__(self, *, settings: Settings) -> None:
        ...
        self._queue_urls = dict(settings.sqs_job_queue_urls)

    def enqueue(self, job: JobEnvelope, *, queue: str) -> EnqueueHandle:
        queue_url = resolve_target_for_queue(
            queue=queue,
            mapping=self._queue_urls,
            fallback=self._queue_url,
            target_name="sqs queue url",
        )

        def _send_once() -> EnqueueHandle:
            body = job.model_dump_json()
            params: dict[str, str] = {"QueueUrl": queue_url, "MessageBody": body}
        ...
        return send_with_retry(
            ...
            queue=queue,
            job=job,
            max_retries=self._settings.job_enqueue_max_retries,
            backoff_seconds=self._settings.job_enqueue_retry_backoff_seconds,
