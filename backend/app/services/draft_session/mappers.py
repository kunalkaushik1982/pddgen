...
def _llm_usage_totals(session: DraftSessionModel) -> tuple[int, int, int, int]:
    """Return (calls, prompt, completion, total) for one session."""
    events = getattr(session, "llm_usage_events", []) or []
    calls = len(events)
    prompt = sum(int(getattr(item, "prompt_tokens", 0) or 0) for item in events)
    completion = sum(int(getattr(item, "completion_tokens", 0) or 0) for item in events)
    total_reported = sum(int(getattr(item, "total_tokens", 0) or 0) for item in events)
    if total_reported <= 0 and (prompt > 0 or completion > 0):
        total_reported = prompt + completion
...
def map_artifact(artifact: ArtifactModel, storage_service=None) -> ArtifactResponse:
    ...
    return ArtifactResponse(
        ...
        name=artifact.name,
        kind=cast(ArtifactKind, artifact.kind),
...
def map_process_step(step: ProcessStepModel, storage_service=None) -> ProcessStepResponse:
    ...
    return ProcessStepResponse(
        ...
        confidence=cast(ConfidenceLevel, step.confidence),
...
def map_draft_session(session: DraftSessionModel) -> DraftSessionResponse:
    ...
    return DraftSessionResponse(
        ...
        document_type=cast(WorkflowDocumentType, session.document_type),
...
def map_draft_session_list_item(session: DraftSessionModel) -> DraftSessionListItemResponse:
    ...
    llm_call_count, total_prompt_tokens, total_completion_tokens, total_tokens_reported = _llm_usage_totals(session)
