...
@dataclass
class _ScopedSessionView:
    """Narrow subset of ``DraftSessionModel`` fields used by diagram helpers for one process group."""
    ...
    process_steps: list
    process_notes: list
    diagram_layouts: list
    overview_diagram_json: str
    detailed_diagram_json: str


class _OverviewBucket(TypedDict):
...
class ProcessDiagramService:
    ...
    def build_detailed_flowchart_model(self, draft_session) -> dict:
        ...
        for index, step in enumerate(normalized_steps, start=1):
            ...
            nodes.append(
                {
                    "id": step_id,
                    "label": self._shorten(str(step["label"]), 10),
                    "category": "decision" if step["is_decision"] else "process",
                    "step_range": str(step["step_range"]),
                }
            )
            if next_id:
    ...
    def _scope_session(self, draft_session, process_group_id: str | None):
        ...
        return _ScopedSessionView(
            id=draft_session.id,
            title=scoped_title,
            diagram_type=getattr(draft_session, "diagram_type", None),
            process_steps=[
                step
                for step in getattr(draft_session, "process_steps", [])
                if getattr(step, "process_group_id", None) == process_group_id
            ],
            process_notes=[
                note
                for note in getattr(draft_session, "process_notes", [])
                if getattr(note, "process_group_id", None) == process_group_id
            ],
            diagram_layouts=[
                layout
            ...
            overview_diagram_json=overview_json,
            detailed_diagram_json=detailed_json,
        )
    ...
    def render_flowchart_view(
        ...
        view_type: str,
        output_path: Path,
        saved_positions: dict[str, dict[str, float | str]] | None = None,
    ...
    def _text_height(draw: ImageDraw.ImageDraw, lines: list[str], font) -> int:
        return int(
            sum(
                int(draw.textbbox((0, 0), line, font=font)[3]) - int(draw.textbbox((0, 0), line, font=font)[1])
                for line in lines
            )
            + max(0, len(lines) - 1) * 18
        )
    ...
    def _build_overview_business_nodes(self, ordered_steps) -> list[dict[str, str | bool]]:
        grouped_by_title: dict[str, _OverviewBucket] = {}
