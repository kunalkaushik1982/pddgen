...
class _LlmSessionAgg(TypedDict):
    """Aggregated LLM usage per session row (see ``list_admin_session_metrics``)."""
...
def list_admin_session_metrics(db: Session, *, owner_id: str | None = None) -> list[AdminSessionMetricsResponse]:
    ...
    llm_map: dict[str, _LlmSessionAgg] = {}
    for row in llm_rows:
        tts = row[3]
        llm_map[str(row[0])] = {
            "prompt": int(row[1] or 0),
            "completion": int(row[2] or 0),
            "total_tokens_sum": int(tts) if tts is not None else None,
            "cost": row[4],
    ...
    for s in sessions:
        lm = llm_map.get(s.id, _EMPTY_LLM_AGG)
        tp = lm["prompt"]
        tc = lm["completion"]
        tts = lm["total_tokens_sum"]
        total_rep = tts
        cost_val = lm["cost"]
